...
static void spi1_init(void) {
    ...
    GPIOB->ODR |= (1 << BMI088_ACC_CS_PIN) | (1 << BMI088_GYRO_CS_PIN);  // CS high
...
void bmi088_acc_cs_low(void)  { GPIOB->ODR &= ~(1 << BMI088_ACC_CS_PIN); }
void bmi088_acc_cs_high(void) { GPIOB->ODR |= (1 << BMI088_ACC_CS_PIN); }
void bmi088_gyro_cs_low(void)  { GPIOB->ODR &= ~(1 << BMI088_GYRO_CS_PIN); }
void bmi088_gyro_cs_high(void) { GPIOB->ODR |= (1 << BMI088_GYRO_CS_PIN); }
uint8_t bmi088_spi_transfer(uint8_t data) { return spi1_transfer(data); }
void bmi088_delay_us(uint32_t us) { platform_delay_us(us); }
void bmi088_delay_ms(uint32_t ms) { platform_delay_ms(ms); }
...
bool bmp388_i2c_read(uint8_t addr, uint8_t reg, uint8_t *data, uint8_t len) {
    if (!i2c3_write(addr, &reg, 1)) return false;
...
void bmp388_delay_ms(uint32_t ms) { platform_delay_ms(ms); }
...
bool vl53l1x_i2c_read(uint8_t addr, uint16_t reg, uint8_t *data, uint16_t len) {
    ...
    if (!i2c3_write(addr, reg_buf, 2)) return false;
    return i2c3_read(addr, data, len);
}

bool vl53l1x_i2c_write(uint8_t addr, uint16_t reg, uint8_t *data, uint16_t len) {
    uint8_t buf[len + 2];
    buf[0] = (uint8_t)(reg >> 8);
    buf[1] = (uint8_t)(reg & 0xFF);
    for (uint16_t i = 0; i < len; i++) buf[i+2] = data[i];
    return i2c3_write(addr, buf, len + 2);
}

void vl53l1x_delay_ms(uint32_t ms) { platform_delay_ms(ms); }

// ----------------------------------------------------------------------------
// SPI Interface for PMW3901 (Flow deck)
// ----------------------------------------------------------------------------

// PMW3901 SPI callbacks (stubbed - need to implement when deck pinout confirmed)
void pmw3901_cs_low(void)  { GPIOB->ODR &= ~(1 << FLOW_SPI_CS_PIN); }
void pmw3901_cs_high(void) { GPIOB->ODR |= (1 << FLOW_SPI_CS_PIN); }
uint8_t pmw3901_spi_transfer(uint8_t data) { return spi1_transfer(data); }
void pmw3901_delay_us(uint32_t us) { platform_delay_us(us); }
void pmw3901_delay_ms(uint32_t ms) { platform_delay_ms(ms); }

// ----------------------------------------------------------------------------
// LED Control
// ----------------------------------------------------------------------------

void platform_led_on(void)     { LED_PORT->ODR |= LED_PIN; }
void platform_led_off(void)    { LED_PORT->ODR &= ~LED_PIN; }
void platform_led_toggle(void) { LED_PORT->ODR ^= LED_PIN; }
