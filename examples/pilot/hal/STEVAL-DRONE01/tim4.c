...
static volatile uint32_t *const s_ccr[4] = {&TIM4_CCR1, &TIM4_CCR2, &TIM4_CCR3,
                                            &TIM4_CCR4};
...
static void init_gpio_pb8_pb9(void) {
    ...
    // These are already configured by gpio_init_tim4_pwm() but we call it
    // anyway
...
void tim4_init(const tim4_config_t *config) {
    ...
    uint32_t prescaler =
        (tim_clk / ((uint32_t)s_config.frequency * TIM4_PWM_RESOLUTION)) - 1;
