...
static void switch_actor_a(void *args, const hive_spawn_info_t *siblings,
    ...
    while (ctx->count < ctx->max_count) {
        // Notify ping to B
        int msg = 1;
        hive_ipc_notify(ctx->partner, HIVE_ID_NONE, &msg, sizeof(msg));
...
static void switch_actor_b(void *args, const hive_spawn_info_t *siblings,
    ...
    while (ctx->count < ctx->max_count) {
        ...
        int reply = 2;
        hive_ipc_notify(ctx->partner, HIVE_ID_NONE, &reply, sizeof(reply));
...
static void bench_context_switch(void) {
    ...
    hive_spawn(switch_actor_b, NULL, ctx_b, NULL, &b);
    hive_spawn(switch_actor_a, NULL, ctx_a, NULL, &a);
    ctx_a->partner = b;
    ctx_b->partner = a;
...
static void ipc_sender(void *args, const hive_spawn_info_t *siblings,
    ...
    for (uint64_t i = 0; i < ctx->max_count; i++) {
        hive_ipc_notify(ctx->partner, HIVE_ID_NONE, buffer, ctx->msg_size);
...
static void ipc_receiver(void *args, const hive_spawn_info_t *siblings,
    ...
    for (uint64_t i = 0; i < ctx->max_count; i++) {
        ...
        // Notify ack
        hive_ipc_notify(ctx->partner, HIVE_ID_NONE, &ack, sizeof(ack));
    }
...
static void bench_ipc_copy(size_t msg_size, const char *label) {
    ...
    hive_spawn(ipc_receiver, NULL, ctx_recv, NULL, &recv);
    hive_spawn(ipc_sender, NULL, ctx_send, NULL, &send);
