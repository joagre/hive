...
static void server_actor(void *args, const hive_spawn_info_t *siblings,
    ...
    // Listen on port
    hive_status_t status = hive_tcp_listen(TEST_PORT, &g_listen_fd);
    ...
    status =
        hive_tcp_accept(g_listen_fd, &g_accepted_fd, 2000); // 2 second timeout
    if (HIVE_FAILED(status)) {
    ...
    size_t received = 0;
    status =
        hive_tcp_recv(g_accepted_fd, buf, sizeof(buf) - 1, &received, 2000);
    if (HIVE_FAILED(status)) {
        ...
        // Echo back
        size_t sent = 0;
        hive_tcp_send(g_accepted_fd, buf, received, &sent, 2000);
    }
...
static void client_actor(void *args, const hive_spawn_info_t *siblings,
    ...
    // Connect to server
    int fd = -1;
    hive_status_t status = hive_tcp_connect("127.0.0.1", TEST_PORT, &fd, 2000);
    if (HIVE_FAILED(status)) {
    ...
    size_t sent = 0;
    status = hive_tcp_send(fd, data, strlen(data), &sent, 2000);
    if (HIVE_FAILED(status)) {
    ...
    char buf[64] = {0};
    size_t received = 0;
    status = hive_tcp_recv(fd, buf, sizeof(buf) - 1, &received, 2000);
    if (HIVE_FAILED(status)) {
...
static void echo_server_actor(void *args, const hive_spawn_info_t *siblings,
                              size_t sibling_count) {
    (void)args;
    (void)siblings;
    (void)sibling_count;
    ...
    // Receive and store
    status = hive_tcp_recv(conn_fd, s_received_data,
    ...
    if (HIVE_SUCCEEDED(status)) {
        ...
        size_t sent = 0;
        hive_tcp_send(conn_fd, reply, strlen(reply), &sent, 2000);
    }
...
static void echo_client_actor(void *args, const hive_spawn_info_t *siblings,
                              size_t sibling_count) {
    (void)args;
    (void)siblings;
    (void)sibling_count;
    ...
    size_t sent = 0;
    hive_tcp_send(fd, data, strlen(data), &sent, 2000);

    // Receive reply
    size_t received = 0;
    status = hive_tcp_recv(fd, s_echo_reply, sizeof(s_echo_reply) - 1,
                           &received, 2000);
    if (HIVE_SUCCEEDED(status)) {
...
static void test3_accept_timeout(void *args, const hive_spawn_info_t *siblings,
                                 size_t sibling_count) {
    (void)args;
    (void)siblings;
    (void)sibling_count;
    ...
    int conn_fd = -1;
    status = hive_tcp_accept(listen_fd, &conn_fd, 100); // 100ms timeout
...
static void test4_connect_invalid(void *args, const hive_spawn_info_t *siblings,
                                  size_t sibling_count) {
    (void)args;
    (void)siblings;
    (void)sibling_count;
    ...
    hive_status_t status =
        hive_tcp_connect("127.0.0.1", 1, &fd, 500); // Port 1 typically fails
...
static void test5_short_timeout_accept(void *args,
    ...
    int conn_fd = -1;
    status = hive_tcp_accept(listen_fd, &conn_fd, 10);
...
static void test6_close_reuse(void *args, const hive_spawn_info_t *siblings,
                              size_t sibling_count) {
    (void)args;
    (void)siblings;
    (void)sibling_count;
...
static void test7_nonblocking_accept(void *args,
    ...
    status = hive_tcp_accept(listen_fd, &conn_fd, 0);
...
static void test8_recv_timeout(void *args, const hive_spawn_info_t *siblings,
                               size_t sibling_count) {
    (void)args;
    (void)siblings;
    (void)sibling_count;
    ...
    int client_fd = -1;
    status = hive_tcp_connect("127.0.0.1", TEST_PORT + 6, &client_fd, 1000);
    if (HIVE_FAILED(status)) {
    ...
    int server_fd = -1;
    status = hive_tcp_accept(listen_fd, &server_fd, 1000);
    if (HIVE_FAILED(status)) {
    ...
    size_t received = 0;
    uint64_t start = time_ms();
    status = hive_tcp_recv(server_fd, buf, sizeof(buf), &received,
                           100); // 100ms timeout
    uint64_t elapsed = time_ms() - start;
...
static void test9_nonblocking_recv(void *args,
                                   const hive_spawn_info_t *siblings,
                                   size_t sibling_count) {
    (void)args;
    (void)siblings;
    ...
    int client_fd = -1;
    status = hive_tcp_connect("127.0.0.1", TEST_PORT + 7, &client_fd, 1000);
    if (HIVE_FAILED(status)) {
        TEST_FAIL("connect failed");
        hive_tcp_close(listen_fd);
        return;
    ...
    size_t received = 0;
    uint64_t start = time_ms();
    status = hive_tcp_recv(server_fd, buf, sizeof(buf), &received, 0);
    uint64_t elapsed = time_ms() - start;
...
static void test10_nonblocking_send(void *args,
                                    const hive_spawn_info_t *siblings,
                                    size_t sibling_count) {
    (void)args;
    (void)siblings;
    ...
    int client_fd = -1;
    status = hive_tcp_connect("127.0.0.1", TEST_PORT + 8, &client_fd, 1000);
    if (HIVE_FAILED(status)) {
        TEST_FAIL("connect failed");
        hive_tcp_close(listen_fd);
        return;
    ...
    size_t sent = 0;
    status = hive_tcp_send(client_fd, data, strlen(data), &sent, 0);
...
static void test11_connect_timeout(void *args,
                                   const hive_spawn_info_t *siblings,
                                   size_t sibling_count) {
    (void)args;
    (void)siblings;
    ...
    uint64_t start = time_ms();
    hive_status_t status =
        hive_tcp_connect("10.255.255.1", 12345, &fd, 200); // 200ms timeout
    uint64_t elapsed = time_ms() - start;
...
static void blocked_recv_actor(void *args, const hive_spawn_info_t *siblings,
                               size_t sibling_count) {
    (void)siblings;
    (void)sibling_count;
    int fd = *(int *)args;
...
static void test12_actor_death_during_recv(void *args,
    ...
    int client_fd = -1;
    status = hive_tcp_connect("127.0.0.1", TEST_PORT + 10, &client_fd, 1000);
    if (HIVE_FAILED(status)) {
        TEST_FAIL("connect failed");
        hive_tcp_close(listen_fd);
        return;
