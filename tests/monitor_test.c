...
static void target_normal_exit(void *args, const hive_spawn_info_t *siblings,
...
static void test3_multi_monitor_actor(void *args,
    ...
    while (received < 3) {
        hive_message_t msg;
        hive_status_t status = hive_ipc_recv(&msg, 2000); // 2 second timeout
        if (HIVE_FAILED(status)) {
...
static void target_waits_for_exit(void *args, const hive_spawn_info_t *siblings,
                                  size_t sibling_count) {
    (void)args;
    (void)siblings;
    (void)sibling_count;
...
static void test9_monitor_pool_exhaustion(void *args,
    ...
    for (int i = 0; i < HIVE_MONITOR_ENTRY_POOL_SIZE + 10; i++) {
        ...
        hive_status_t status = hive_monitor(target, &refs[monitored]);
        if (HIVE_FAILED(status)) {
